...
public class Model {
	...
	public double [][] alpha_et, alpha_ts;
	public double [] alphaSum_e, alphaSum_t;
	...
	public double [][] theta_e; //theta: document - topic distributions, size M x E
	public double [][][] theta_s, theta_t; //size M x T x S, M x E x T
	...
	protected int [][][] ndet, ndts; //ndet[i][j][k]: number of words in document i assigned to entity j of topic k ,  size M x E x T 
	...
	protected int [][] nde, ndt; //ndetsum[i][j]: number of words in document i assigned to entity j of topic k, size M x E
	...
	public boolean initNewModel(LDACmdOption option){
			...
			for (int n = 0; n < N; n++){
				...
				for(List<String> sentimentList:initialSentimentWord){
					...
					for(String sentimentWord:sentimentList){
						if(token.equals(sentimentWord.split("\\s+")[0].toLowerCase())){
							sentiment = i;
							break;
						}
					}
				...
				*/
				...
				nde[m][entity] += 1;
				ndt[m][topic] += 1;
				ndts[m][topic][sentiment] += 1;
				ndet[m][entity][topic] += 1;
				nd[m] += 1;
			}
		...
		theta_e = new double[M][E];
		theta_t = new double[M][E][T];
		theta_s = new double[M][T][S];
		...
		alpha_et = new double[E][T];
		alphaSum_e = new double[E];
		
		for(int e = 0; e < E; e++){
			alphaSum_e[e] = 0;
			for(int t = 0; t < T; t++){
				alpha_et[e][t] = alpha_t;
				alphaSum_e[e] += alpha_et[e][t];
				
			}
		}
		
		alpha_ts = new double[T][S];
		alphaSum_t = new double[T];
		
		for(int t = 0; t < T; t++){
			alphaSum_t[t] = 0;
			for(int s = 0; s < S; s++){
				alpha_ts[t][s] = alpha_t;
				alphaSum_t[t] += alpha_ts[t][s];
	...
	public void loadPriorDataMPQA(String topiclist){
		List posTopics=new ArrayList();
		List negTopics=new ArrayList();
		List neuTopics=new ArrayList();
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(topiclist), "UTF-8"));
			String line = null;
			while ((line = reader.readLine()) != null) {
				...
				if(values[1].equals(0.9)) {
					posTopics.add(values[0]);
				}else if (values[2].equals(0.9)) {
					negTopics.add(values[0]);
				}else if(values[3].equals(0.9)) {
					neuTopics.add(values[0]);
				}
		...
		initialSentimentWord.add(neuTopics);
	...
	public void prior2beta(){
		...
		while (it3.hasNext()){
			...
			Integer value = wordmap.get(key);
			for(int s = 0; s < S; s++){
				if(initialSentimentWord.get(s).toString().split("\\s+")[0].contains(key)){
					for(int j = 0; j < S; j++){
						if(j != s){
							lambda_s[j][value] = 0;
														
						} else if (initialSentimentWord.get(s).toString().split("\\s+")[1].equals("trust") || initialSentimentWord.get(s).toString().split("\\s+")[1].equals("anger")) {
							lambda_s[j][value] = 0.8;
						} else if (initialSentimentWord.get(s).toString().split("\\s+")[1].equals("joy") || initialSentimentWord.get(s).toString().split("\\s+")[1].equals("disgust")) {
							lambda_s[j][value] = 0.6;
						} else if (initialSentimentWord.get(s).toString().split("\\s+")[1].equals("anticip") || initialSentimentWord.get(s).toString().split("\\s+")[1].equals("fear")) {
							lambda_s[j][value] = 0.4;
						} else if (initialSentimentWord.get(s).toString().split("\\s+")[1].equals("sandness")) {
