...
public class Estimator {
	// output model
		...
		public int[] sampling(int m, int n){
			...
			trnModel.ndts[m][topic][sentiment] -= 1;
				...
				for (int t = 0; t < trnModel.T; t++) {
					for (int s = 0; s < trnModel.S; s++) {
						trnModel.p[e][t][s] = (trnModel.nwe[w][e] + trnModel.beta_ew[e][w])/(trnModel.ne[e] + trnModel.beta_esum[e]) *
								...
								(trnModel.ndet[m][e][t] + trnModel.alpha_et[e][t])/(trnModel.nde[m][e] + trnModel.alphaSum_e[e]) *
								(trnModel.ndts[m][t][s] + trnModel.alpha_ts[t][s])/(trnModel.ndt[m][t] + trnModel.alphaSum_t[t]);
