...
public class Inferencer {
	...
	//inference new model ~ getting dataset from file specified in option
		...
		protected int[] infSampling(int m, int n){
			...
			newModel.ndets[m][entity][topic][sentiment] -= 1;
			...
			double Talpha = newModel.alpha_t * newModel.T;
			...
			if(newModel.data.lid2gid.containsKey(_w)) {
					...
					for (int t = 0; t < newModel.T; t++) {
						for (int s = 0; s < newModel.S; s++) {
							newModel.p[e][t][s] = (trnModel.nwe[w][e] + newModel.nwe[_w][e] + newModel.beta_ew[e][_w])/(trnModel.ne[e] + newModel.ne[e] + newModel.beta_esum[e]) *
									...
									(newModel.ndt[m][t] + newModel.alpha_t)/(newModel.nd[m] + Talpha) *
									(newModel.ndets[m][e][t][s] + newModel.alpha_ets[e][t][s])/(newModel.ndet[m][e][t] + newModel.alphaSum_et[e][t]);
						}
					}
				...
				for (int e = 0; e < newModel.E; e++){
					for (int t = 0; t < newModel.T; t++) {
						for (int s = 0; s < newModel.S; s++) {
							newModel.p[e][t][s] = (newModel.nwe[_w][e] + newModel.beta_ew[e][_w])/(newModel.ne[e] + newModel.beta_esum[e]) *
									(newModel.nwt[_w][t] + newModel.beta_tw[t][_w])/(newModel.nt[t] + newModel.beta_tsum[t]) *
		...
		protected void computeNewTheta(){
			...
			for (int m = 0; m < newModel.M; m++){
				for (int t = 0; t < newModel.T; t++){
					newModel.theta_t[m][t] = (newModel.ndt[m][t] + newModel.alpha_t) / (newModel.nd[m] + newModel.T * newModel.alpha_t);
				}
			}
			...
			for (int m = 0; m < newModel.M; m++){
				for (int e = 0; e < newModel.E; e++){
					for (int t = 0; t < newModel.T; t++){
						for (int s = 0; s < newModel.S; s++) {
							newModel.theta_s[m][e][t][s] = (newModel.ndets[m][e][t][s] + newModel.alpha_ets[e][t][s]) / (newModel.ndet[m][e][t] + newModel.alphaSum_et[e][t]);
						}
					}
				}
			}
		...
		protected void computeNewPhi(){
			...
			for (int e = 0; e < newModel.E; e++){
				for (int w = 0; w < newModel.V; w++){
					...
					if (id != null) {
						newModel.phi_e[e][w] = (trnModel.nwe[id][e] +newModel.nwe[w][e] + newModel.beta_ew[e][w]) / (trnModel.ne[e] + newModel.ne[e] + newModel.beta_esum[e]);
					}else {
			...
			for (int t = 0; t < newModel.T; t++){
				for (int w = 0; w < newModel.V; w++){
					Integer id = newModel.data.lid2gid.get(w);
					if (id != null) {
						newModel.phi_t[t][w] = (trnModel.nwt[id][t] +newModel.nwt[w][t] + newModel.beta_tw[t][w]) / (trnModel.nt[t] + newModel.nt[t] + newModel.beta_tsum[t]);
					}else {
