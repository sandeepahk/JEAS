...
public class Model {
	...
	public double [][][] alpha_ets;
	public double [][] alphaSum_et;
	...
	protected int [][] nde, ndt; //nd[i][j]: number of words in document i assigned to topic j, size M x E,  M x T
	...
	protected int [][][][] ndets; //ndet_s[i][j][k][l]: number of words in document i assigned to entity j of topic k of sentiment l,  size M x E x T x S
	protected int [][][] ndet; //ndetsum_s[i][j][k]: number of words in document i assigned to entity j of topic k, size M x E x T
		...
		*/
		...
		for (int m = 0; m < data.M; m++){
			...
			for (int n = 0; n < N; n++){
				...
				nde[m][entity] += 1;
				ndt[m][topic] += 1;
				ndets[m][entity][topic][sentiment] += 1;
		...
		theta_t = new double[M][T];
		theta_s = new double[M][E][T][S];
		...
		alpha_ets = new double[E][T][S];
		alphaSum_et = new double[E][T];
		...
		for(int e = 0; e < E; e++){
			for(int t = 0; t < T; t++){
				...
				for(int s = 0; s < S; s++){
					...
					alpha_ets[e][t][s] = alpha_s;
					alphaSum_et[e][t] += alpha_ets[e][t][s];
				}
			}
		}
